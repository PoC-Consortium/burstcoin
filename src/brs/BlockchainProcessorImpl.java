...
final class BlockchainProcessorImpl implements BlockchainProcessor {
  ...
  /*
   * private static void addCollectionGauge (MetricRegistry metrics, final Collection c, String name) { metrics.register(MetricRegistry.name(BlockchainProcessorImpl.class, name, "size"), (Gauge<Integer>) () -> c.size()); }
   * 
   * static { // addCollectionGauge(Burst.metrics, blockCache.keySet(), "BlockCache"); Disabled for now // addCollectionGauge(Burst.metrics, unverified, "Unverified"); // Burst.metrics.register(MetricRegistry.name(BlockchainProcessorImpl.class, "BlockCacheSize", "size"), // (Gauge<Integer>) () ->
   * DownloadCache.blockCacheSize); }
   * 
   */
  ...
  public void fullReset() {
    synchronized (blockchain) {
      // blockDb.deleteBlock(Genesis.GENESIS_BLOCK_ID); // fails with stack
      // overflow in H2
      blockDb.deleteAll();
      addGenesisBlock();
      scan(0);
  ...
  private void pushBlock(final BlockImpl block) throws BlockNotAcceptedException {
    ...
    try {
      ...
      synchronized (blockchain) {
        // logger.debug("Trying to push block " + block.getId() + " (height " +
        // block.getHeight() + ")");
        ...
        try {
          ...
          for (TransactionImpl transaction : block.getTransactions()) {
            ...
            /*
             * if (!transaction.verifySignature()) { // moved to preVerify throw new TransactionNotAcceptedException( "Signature verification failed for transaction " + transaction.getStringId() + " at height " + previousLastBlock.getHeight(), transaction); }
             */
  ...
  public void scan(int height) {
    ...
    // logger.info("Rollback of more than " + Constants.MAX_ROLLBACK + " blocks
    ...
    //
    // PreparedStatement pstmt = con.prepareStatement("SELECT * FROM block WHERE
    // height >= ? ORDER BY db_id ASC")) {
    // transactionProcessor.requeueAllUnconfirmedTransactions();
    // Account.flushAccountTable();
    // for (DerivedTable table : derivedTables) {
    // if (height == 0) {
    ...
    // blockListeners.notify(currentBlock,
    // BlockchainProcessor.Event.RESCAN_BEGIN);
    // long currentBlockId = currentBlock.getId();
    // if (height == 0) {
    // blockchain.setLastBlock(currentBlock); // special case to avoid no last
    // block
    ...
    // }
    // throw new BurstException.NotValidException("Database blocks in the wrong
    // order!");
    // }
    ...
    // throw new BurstException.NotValidException("Invalid block generation
    // signature");
    // }
    ...
    // BlockImpl.parseBlock(blockJSON).getBytes())) {
    // throw new BurstException.NotValidException("Block JSON cannot be parsed
    // back to the same block");
    // }
    // for (TransactionImpl transaction : currentBlock.getTransactions()) {
    // /*if (!transaction.verifySignature()) { // moved to preVerify
    // throw new BurstException.NotValidException("Invalid transaction
    // signature");
    // }*/
    // if (!transaction.verifyPublicKey()) {
    // throw new BurstException.NotValidException("Wrong transaction public
    // key");
    // }
    ...
    // /*
    // if (!EconomicClustering.verifyFork(transaction)) {
    // logger.debug("Found transaction that was generated on a fork: " +
    // transaction.getStringId()
    // + " in block " + currentBlock.getStringId() + " at height " +
    ...
    // throw new BurstException.NotValidException("Transaction bytes cannot be
    // parsed back to the same transaction");
    // }
    ...
    // throw new BurstException.NotValidException("Transaction JSON cannot be
    // parsed back to the same transaction");
    // }
    // }
    // }
    ...
    // logger.debug("Applying block " + Convert.toUnsignedLong(currentBlockId) +
    // " at height "
    // + (currentBlock == null ? 0 : currentBlock.getHeight()) + " failed,
