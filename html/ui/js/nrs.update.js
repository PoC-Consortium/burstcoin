...
var NRS = (function(NRS, $, undefined) {
        ...
        if (NRS.inApp) {
            if (NRS.appPlatform && NRS.appVersion) {
                ...
                if (new Date() > noticeDate) {
                    ...
                    $("#secondary_dashboard_message").removeClass("alert-success").addClass("alert-danger").html($.t("old_burst_wallet_update", {
                        "link": downloadUrl
                    })).show();
                }
            }
    ...
    NRS.checkForNewVersion = function() {
        ...
        $(".brs_new_version_nr").html(NRS.normalVersion.versionNr).show();
        $(".brs_beta_version_nr").html(NRS.betaVersion.versionNr).show();
    ...
    NRS.verifyClientUpdate = function(e) {
        ...
        worker.onmessage = function(e) {
            if (e.data.progress) {
                ...
                $("#brs_update_hash_version").html(NRS.downloadedVersion.versionNr);
                $("#brs_update_hash_download").html(e.data.sha256);
                $("#brs_update_hash_official").html(NRS.downloadedVersion.hash);
    ...
    NRS.downloadClientUpdate = function(version) {
        ...
        if (NRS.inApp) {
            ...
            $("#brs_update_iframe").attr("src", "https://github.com/PoC-Consortium/burstcoin/releases/download/" + NRS.downloadedVersion.versionNr + "/burstcoin-" + NRS.downloadedVersion.versionNr + ".zip");
